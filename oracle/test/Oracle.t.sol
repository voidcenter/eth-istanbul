...
contract OracleTest is Test {
    ...
    RepOracleContract public oracle_bare;
    DemoRepOracleUser public user_bare;
    ...
    function setUp() public {
        // testing on Goerli
        // oracle = new RepOracleContract(UMAOracle, axiomv2query, true, true);
        // user = new DemoRepOracleUser(address(oracle));
        
        oracle_bare = new RepOracleContract(address(0), address(0), false, false);
        user_bare = new DemoRepOracleUser(address(oracle_bare));
    }
    ...
    function test_SingleRequest() public {
     ...
     function test_no_uma_no_axiom() public {
        // Setup
        uint256 beginningNonce = oracle_bare.requestNonce();
        address queryingAddress = address(1);
        ...
        // Sending Request through user contrat
        bytes32 requestId = oracle_bare.requestReputationScore(queryingAddress, true, 0);

        assertEq(oracle_bare.requestNonce(), beginningNonce + 1);

        assertEq(oracle_bare.call_back_address(expectedRequestID), address(this));
        ...
        bytes32 assertionId =
            oracle_bare.sendReputationScore(exampleScore, "verify IPFS hash QmQMvd3pSWxaoEVit5oui9xcJa4C5UqZLAcubuhawcN3F6");
